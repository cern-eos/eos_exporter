	...
	return &VSCollector{
		...
		Uptime: prometheus.NewGaugeVec(
			prometheus.GaugeOpts{
				Namespace:   namespace,
				Name:        "vs_uptime",
				Help:        "Uptime: Amoujnt of time the FST has been up",
				ConstLabels: labels,
			},
			[]string{"mgm_version","node","geotag","eos_v_fst","xrd_v_fst","kernel_v"},
		),
...
func (o *VSCollector) collectVSDF() error {
    ...
    mds, err := client.ListVS(context.Background(), "root")
    if err != nil {
	panic(err)
    }
	...
	if err == nil {
		o.Vsize.WithLabelValues(m.EOSmgm,m.Hostport,m.Geotag,m.EOSfst,m.Xrootdfst,m.KernelV).Set(vsize)
	}

	// Rss
	rss, err := strconv.ParseFloat(m.Rss, 64)
	if err == nil {
		o.Rss.WithLabelValues(m.EOSmgm,m.Hostport,m.Geotag,m.EOSfst,m.Xrootdfst,m.KernelV).Set(rss)
	}
	...
	if err == nil {
		o.Threads.WithLabelValues(m.EOSmgm,m.Hostport,m.Geotag,m.EOSfst,m.Xrootdfst,m.KernelV).Set(threads)
	}
	...
	if err == nil {
		o.Sockets.WithLabelValues(m.EOSmgm,m.Hostport,m.Geotag,m.EOSfst,m.Xrootdfst,m.KernelV).Set(sockets)
	}
	...
	if err == nil {
		o.Uptime.WithLabelValues(m.EOSmgm,m.Hostport,m.Geotag,m.EOSfst,m.Xrootdfst,m.KernelV).Set(uptime)
	}
    }
